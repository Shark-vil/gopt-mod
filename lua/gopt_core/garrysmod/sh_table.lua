...
local function IsArray(t)
	...
	local lenght = #t
	return lenght > 0 and next(t, lenght) == nil
end
...
function table.Random(t)
	local rk = math_random(1, table.Count(t))
	local i = 1
...
local function MakeTable(t, nice, indent, done)
	...
	local idt = ""
	local MakeTable = MakeTable

	if nice then
		idt = string_rep("\t", indent)
	end
...
function table.DeSanitise(t, done)
	local done = done or {}
	local tbl = {}
	local table_DeSanitise = table.DeSanitise
...
function table.CollapseKeyValue(Table)
	local OutTable = {}
	local table_CollapseKeyValue = table.CollapseKeyValue

	for k, v in pairs(Table) do
		local Val = v.Value

		if istable(Val) then
			Val = table_CollapseKeyValue(Val)
		end
